...
class DampenSpringSystem():
    ...
    def generate_noisy_datapoints(self, t_min, t_max, num_points, std_x, true_derivatives = True):
        ...
        return datadict
    def estimate_lipschitz_constant(self, t_min, t_max, n_samples=1000):
        """
        Estimate the Lipschitz constant for the system using its first derivative.
        ...
        derivative_values = [self.get_first_derivatives(t) for t in t_samples]
        return max(np.abs(derivative_values))
